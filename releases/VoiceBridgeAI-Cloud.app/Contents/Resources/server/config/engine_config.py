...
from config.asr_config import get_status as asr_status
from config.asr_config import normalize_mode as normalize_asr
from config.asr_config import set_provider as set_asr
from config.final_config import get_status as final_status
from config.final_config import set_provider as set_final
from config.partial_config import get_status as partial_status
from config.partial_config import set_provider as set_partial


def apply_settings(payload: dict[str, Any]) -> None:
    from config.env_persist import persist_engine_config
    from config.final_config import available_providers as final_available
    from config.partial_config import normalize_provider as norm_partial
    from providers.provider_registry import resolve_final_provider
...
def get_engine_status(asr_mode: str | None = None) -> dict[str, Any]:
    from providers.translate_tmt import configured as tmt_configured
    from providers.translate_tmt import region as tmt_region

    asr = asr_status(asr_mode)
    partial = partial_status()
    final = final_status()

    from providers.provider_registry import LLM_PROVIDERS, REPEAT_MT_PROVIDERS, engine_pair_note, filter_final_providers, resolve_final_provider
    from config.final_config import provider_label
    ...
    if resolved_final != final_id:
        ...
        from config.final_config import engine_label
