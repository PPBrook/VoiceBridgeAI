...
from provider_enable import credentials_ok
from provider_registry import LAYER_PROVIDERS
...
def _test_asr_tencent() -> tuple[bool, str]:
    from tencent_asr import build_ws_url, configured
...
def _test_translate(provider_id: str, *, partial: bool) -> tuple[bool, str]:
    if partial:
        from partial_config import translate

        out = translate(_SAMPLE, provider_id)
        label = "句中翻译"
    else:
        from final_config import translate
